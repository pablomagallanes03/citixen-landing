...
|---|---|---|
| 1 | Navbar | "Soy vecino" (`/soyvecino`), "Soy gobierno" (`/soygobierno`), CTA "Contactanos". **Orden vecino-first + "Pricing" removido del navbar (jun 2026)** — pricing ahora vive al fondo de `/soygobierno` (sigue accesible desde el footer y en `/pricing`). |
...
**Pricing — componente compartido (jun 2026):** los 2 planes viven en **`components/PricingSection.jsx`** (fuente única de datos: `tiers`). Lo usan tanto `/pricing` (con `headingLevel="h1"` + su FAQ de precios) como el fondo de `/soygobierno` (`headingLevel="h2"`). Editar un plan actualiza ambos lugares. **Modo Comunidad** ya no dice "Gratis" sino **"6 meses gratis"** + "Después, licencia anual accesible" (regla: no prometer gratis para siempre). Features incluyen el canal de comunicación oficial (§38), mapa, comentarios; se quitó "Índice de transparencia verificable" (no se vende transparencia como hook).

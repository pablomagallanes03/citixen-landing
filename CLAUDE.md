...
|---|---|---|
| 1 | `Hero.jsx` | H1 "Tus impuestos. Tu ciudad. Tu decisión." (highlight coral en "Tu decisión"). Lead + frase fuerte ("Citixen propone algo simple...") + tagline "Créditos Cívicos para convertir impuestos en decisiones ciudadanas." CTAs: "Quiero participar" (`Link` → `/soyvecino`) y "Conocé más ↓" (→ `#como-funciona`). 3 puntos de confianza (participación real / ciudades transparentes / respaldo ciudadano). Visual: reusa el patrón `.hv-card` (mismo usado en `/soygobierno`) + las clases `.phone-progress-*`/`.phone-supporters-*` de `PhoneMockup` — cero CSS nuevo para la tarjeta "Proyecto destacado" (foto `/mockup/plaza.png` vía `next/image`, 67% de Créditos asignados, 3.482 vecinos). |
...
| 3 | `TaxToProject.jsx` (nuevo, reemplaza `CreditAllocator.jsx` — decisión sep 2026, ver nota abajo) | "De tu impuesto al proyecto". Fórmula `$1 = 1 CC = $1 de decisión` **con disclaimer explícito** ("En este ejemplo. El tipo de cambio real lo fija cada municipio..."). Ejemplo concreto: pagás $100 → 100 CC → 4 proyectos con `Meta: $X / X CC` + `Tu límite: Y CC` cada uno. Sliders interactivos con **tope propio por proyecto** (no un pool que se redistribuye) — así el propio mecanismo demuestra "no podés ponerlos todos en el mismo lugar". Cierra con nota sobre por qué existen los límites (evitar concentración) y que el municipio puede ajustarlos con motivo visible. |
...
| 9 | `RepresentativeDemocracy.jsx` (nuevo) | Banda navy gradiente (mismo `--citixen-gradient` que el Hero). "Elegís representantes cada algunos años. Tu ciudad toma decisiones todos los días." Cierra con "También podemos construir nuevas formas de decidir juntos." |
...
| — | `EarlyAccess.jsx` | Sin cambios — sigue siendo el form real que pega a `POST /api/government/register-light`. Vive después de `FinalCTA`, bajo `#acceso`. |
| — | `Footer.jsx` | Sin cambios. |

**Decisión de fondo (sep 2026):** a pedido explícito del usuario, el concepto central de la sección 3 dejó de ser "priorización libre entre categorías" (`CreditAllocator.jsx`, borrado — nunca llegó a deployarse) y pasó a ser **asignación ciudadana de recursos públicos con metas y límites reales**: pagás → recibís CC → distribuís dentro de límites por proyecto → el proyecto alcanza su meta → el municipio libera los fondos → se ejecuta. Esto es más preciso respecto al modelo real (`maxDonationPerCitizen` por proyecto, §18 CLAUDE.md raíz) que la versión anterior. **Cuidado deliberado:** la equivalencia `$1 = 1 CC` se presenta siempre con el disclaimer "en este ejemplo" — el tipo de cambio real es variable por municipio (§15 CLAUDE.md raíz ya documenta ejemplos como "1 CC = $1.500 ARS"), así que nunca prometerla como regla universal en copy nuevo.

**Recién quedaron sin importar en `index.js` (archivos conservados, reversible):** `WhatIsCitixen.jsx`, `TwoPaths.jsx`. Se suman a la lista histórica de componentes ya desmontados: `HowToStart`, `CivicLoopScroll`, `Trust`, `Problem`, `CommunityMode`, `Deliberation`, `SocialProof`, `Progressive`, `Scenarios`, `Differentiator`, `CivicCreditsExplainer`, `CaseStudyBlock`, `VecinoBanner`, `PricingSection`. Varios de estos (`HowToStart`, `CivicLoopScroll`, `Trust`, `Problem`) inspiraron visualmente los componentes nuevos de la tabla de arriba sin reutilizarse literalmente (contenido distinto).

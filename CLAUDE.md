...
|---|---|---|
| 1 | Navbar | "Soy vecino" (`/soyvecino`), "Soy gobierno" (`/soygobierno`), CTA "Contactanos". **Orden vecino-first. Tema precios removido de la landing por completo (jun 2026)** — no hay sección de precios, ni en navbar, ni footer, ni página standalone. El costo es B2G consultivo (se conversa). Subsiste solo el gancho de valor "6 meses gratis" como propuesta, no como tabla de precios. `/pricing` redirige 308 a `/soygobierno`. |
| 2 | Hero | Sin eyebrow. **H1 (giro jun 2026 — ángulo gobernanza colaborativa):** *"Tu ciudad decide mejor **cuando decidís vos también.**"* (highlight en `--secondary`/celeste vía inline style, el color que tenía la palabra rotativa). **Se eliminó la palabra rotativa "Pagás _[impuestos/multas/tasas]_"** y su maquinaria (`useState`/`useEffect`/`ROTATING_WORDS` borrados de `Hero.jsx`; CSS `.rotating-word*` queda sin uso, conservado). **Historia del cambio:** el H1 original "Pagás [X], ahora también decidí **adónde van**" tocaba el miedo de control del intendente (el Home lo lee también el comprador) y sobre-prometía (el vecino prioriza entre proyectos aprobados, no redirige el presupuesto). El intento intermedio "decidí qué proyectos se hacen" arregló eso pero **rompió la coherencia con la palabra rotativa** ("Pagás multas… qué proyectos se hacen" no engancha) → se soltó el frame entero por uno colaborativo que sirve a ambos lectores sin susto. **El `<title>`/`og:title` SEO conservan el keyword fiscal a propósito** ("Pagás impuestos. Decidís qué proyectos se hacen.") — divergen del H1 visible para no perder discovery. Subhead en **eje participación/relación, SIN mencionar Créditos Cívicos** (decisión jun 2026 — el Hero corrió del mecanismo CC a la relación gobierno-vecino): *"Citixen cambia tu relación con tu ciudad: participás de las decisiones, priorizás los proyectos que importan y ves cómo se hacen realidad."* (Los 3 proof points "Vinculante" / "Proyectos reales" / "Con respaldo municipal" se **removieron jun 2026** — el bloque `hero-proof` ya no se renderiza; CSS `.hero-proof*` conservado sin uso). CTAs: "Activá Citixen en tu ciudad" (→`#acceso`) y "Ver cómo funciona →" (→`#gobierno`). **Visual `hero-visual` (arco decisión → obra, reemplazó el mockup-wallet en jun 2026):** tarjeta de proyecto ejecutado "Plaza San Martín — Barrio Norte" con foto (`/mockup/plaza.png`, LCP `priority`), chip de categoría, 3 métricas (vecinos que priorizaron / CC asignados / % meta financiada), timeline de 4 pasos del ciclo real (priorizado → aprobado por el municipio → en obra → inaugurada) y footer con "vecinos decidiendo" + chip "Pack de evidencia". Cards de fondo desenfocadas: "Ejecutado" / "Recibiendo apoyo". Challenger A/B del H1 archivado en comentario. **Vocabulario: nunca "votar/votación" — los vecinos priorizan/asignan CC.** |
| 3 | WhatIsCitixen | **Reescrito por el GIRO (jun 2026): ya NO es la tabla de 2 modos.** Ahora es la sección "Una sola plataforma" (`#gobierno` anchor): grid de **6 herramientas** (Opiná · **Deliberación con IA** destacada, tarjeta navy gradiente con flag "El diferenciador" · Propuestas · Mi Ciudad · Comunicación institucional · **Créditos Cívicos** con flag de switch "Se activa cuando estés listo"). CSS scoped con `<style jsx>` (clases `tools-grid`/`tool-card`/`tool-card--star`/`tool-card--switch`) — cero CSS global nuevo. Sin precio. La tabla `what-is-table` y sus clases viejas quedaron sin uso. |
| 3.5 | **Deliberation (NUEVO jun 2026)** | Banda navy gradiente (`#deliberacion`) — el spotlight del motor de deliberación con IA. Problema ("¿quién lee 300 respuestas?") → 3 garantías (nada sin cita validado por máquina · ninguna voz se pierde · se vigila a sí mismo) → principio "La IA asiste, nunca decide" → tarjeta-demo (consulta del galpón ferroviario: acuerdo / tensión / voz minoritaria protegida + disclaimer honesto). **Componente reutilizable:** se renderiza en Home y en `/soygobierno` (arriba del catálogo). CSS scoped `<style jsx>` (`delib-*`). |

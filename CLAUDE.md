...
| 3 | `TaxToProject.jsx` (nuevo, reemplaza `CreditAllocator.jsx` — decisión sep 2026, ver nota abajo) | "De tu impuesto al proyecto". Fórmula `$1 = 1 CC = $1 de decisión`, **sin disclaimer de "lo fija cada municipio"** (línea sacada sep 2026 — esa afirmación era incorrecta, ver nota abajo). Ejemplo concreto: pagás $100 → 100 CC → 4 proyectos con `Meta: $X / X CC` + `Tu límite: Y CC` cada uno. Sliders interactivos con **tope propio por proyecto** (no un pool que se redistribuye) — así el propio mecanismo demuestra "no podés ponerlos todos en el mismo lugar". Cierra con nota sobre por qué existen los límites (evitar concentración) y que el municipio puede ajustarlos con motivo visible. |
